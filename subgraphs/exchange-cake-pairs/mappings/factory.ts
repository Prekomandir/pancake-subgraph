...
import { SkalableFactory, Pair, Token, Bundle } from "../generated/schema";
...
export function handlePairCreated(event: PairCreated): void {
  if (
    Address.fromString(CAKE_ADDRESS).notEqual(event.params.token0) &&
    Address.fromString(CAKE_ADDRESS).notEqual(event.params.token1)
  ) {
    return;
  }
  let factory = SkalableFactory.load(FACTORY_ADDRESS);
  if (factory === null) {
    factory = new SkalableFactory(FACTORY_ADDRESS);
